...
- When adding an item, use the conversation context to determine the category. If I say "add coke to it" after you just listed drinks, you must know 'it' refers to the 'Drinks' category. If the category is not obvious from the context, you must infer the most likely category based on the item's name and existing categories. For example, 'Beef Burger' would likely go into a 'Burgers' or 'Main Meals' category. If no relevant category exists, suggest a new one and ask for confirmation before adding the item.
