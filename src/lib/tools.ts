 ...
 */
export async function getBusinessDataForRange(
  startDateStr: string,
  endDateStr: string
): Promise<GetBusinessDataOutput> {
  try {
    ...
    allOrdersSnapshot.forEach((doc) => {
      ...
      if (paymentDate && paymentDate >= startDate && paymentDate <= endDate) {
        ...
        const isOrderFromPeriod =
          order.timestamp.toDate() >= startDate &&
          order.timestamp.toDate() <= endDate;
      ...
      if (
        settledDate &&
        settledDate >= startDate &&
        settledDate <= endDate &&
        orderDate < startDate
      ) {
        // Paying out customer change on a later day reduces that day's available cash
        // only if the change was NOT previously set aside at End-of-Day.
        const changeDelta = Math.max(
          0,
          (order.changeGiven || 0) - (order.pardonedAmount || 0)
        );
        const method = order.paymentMethod;
        const isCashDrawerImpact = method === "cash" || method === "split";
        if (!order.changeSetAside && isCashDrawerImpact && changeDelta > 0) {
          cashSales -= changeDelta;
        }
      }
    });
