...
import { DollarSign, CreditCard, Ban, FileSignature, AlertTriangle, MinusCircle, Hourglass, Landmark, TrendingUp, Coins, Gift } from 'lucide-react';
...
const FinancialSummaryView: React.FC<FinancialSummaryViewProps> = ({ stats, allUnpaidOrdersTotal }) => {
    const today = new Date();

    const totalMiscExpenses = stats.miscCashExpenses + stats.miscMomoExpenses;
    const netRevenue = (stats.cashSales + stats.momoSales + stats.collectionsFromPreviousDays) - totalMiscExpenses - stats.totalPardonedAmount;
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card>
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <StatCard icon={<Coins className="text-muted-foreground" />} title="Change Owed to Customers" value={formatCurrency(stats.changeOwedForPeriod)} description="Total outstanding change for today" />
                        </CardContent>
                         <CardContent>
                            <div className="space-y-4">
                                ...
                                <Card className="bg-green-100 dark:bg-green-900/30 border-green-200 dark:border-green-700">
                                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                        <CardTitle className="text-sm font-medium text-green-800 dark:text-green-200">Net Revenue</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="text-3xl font-bold text-green-700 dark:text-green-300">{formatCurrency(netRevenue)}</div>
                                        <p className="text-xs text-green-600 dark:text-green-400">(Today's Payments + Collections) - Expenses - Pardons</p>
                                    </CardContent>
                                </Card>
                            </div>
                        </CardContent>
