...
const ReconciliationView: React.FC<ReconciliationViewProps> = ({ stats, orders, onBack, setActiveView }) => {
    ...
    const expectedMomo = useMemo(() => (stats?.momoSales || 0) - (stats?.miscMomoExpenses || 0), [stats]);

    const totalExpectedRevenue = expectedCash + expectedMomo;

    const availableCash = useMemo(() => {
        return totalCountedCash - (deductCustomerChange ? stats.changeOwedForPeriod : 0);
    }, [totalCountedCash, deductCustomerChange, stats.changeOwedForPeriod]);

    // If change has been set aside (deductCustomerChange === true), the expected cash
    // that should be in the drawer does NOT include the change owed to customers.
    const displayedExpectedCash = useMemo(() => {
        return expectedCash - (deductCustomerChange ? stats.changeOwedForPeriod : 0);
    }, [expectedCash, deductCustomerChange, stats.changeOwedForPeriod]);

    const cashBalanceDifference = useMemo(() => {
        // Compare what was counted (minus set-aside change if applicable) against the
        // expected cash that should be in the drawer (also excluding set-aside change).
        const counted = availableCash;
        return counted - displayedExpectedCash;
    }, [availableCash, displayedExpectedCash]);

    const momoBalanceDifference = totalCountedMomo - expectedMomo;
    const totalDiscrepancy = cashBalanceDifference + momoBalanceDifference;
    ...
    const handleSaveReport = async () => {
        if (!stats) {
            toast({
                title: "Error",
                description: "No financial data loaded to create a report.",
                type: "error"
            });
            return;
        }
    ...
    const getBalanceStatus = () => {
        ...
        if (isBalanced) {
            ...
            return { color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/50 border-blue-200 dark:border-blue-800', icon: AlertTriangleIcon, text: `Surplus: ${formatCurrency(totalDiscrepancy)}` };
        } else {
            return { color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/50 border-red-200 dark:border-red-800', icon: AlertTriangleIcon, text: `Deficit: ${formatCurrency(Math.abs(totalDiscrepancy))}` };
        }
    };
    ...
    const AdvancedReconciliationModal = () => {
        ...
        return (
            <Dialog open={showAuditModal} onOpenChange={setShowAuditModal}>
                <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col min-h-0">
                    <DialogHeader>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 py-4 min-h-0 flex-1 overflow-auto">
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {checkedOrderIds.size === filteredOrders.length && filteredOrders.length > 0 && (
                                        <Alert>
                                            ...
                                            <UiAlertDescription className="text-sm">
    ...
    const confirmationDescription = `You are about to finalize the financial report for today. ${stats.changeOwedForPeriod > 0
        ? `You have indicated that customer change of ${formatCurrency(stats.changeOwedForPeriod)} has been ${deductCustomerChange ? 'DEDUCTED from the available cash' : 'LEFT IN the cash drawer'}.`
        : ''
                                    ...
                                    <CardContent>
                                        ...
                                        <p className="text-sm text-muted-foreground mt-3">
                                            {deductCustomerChange
                                                ? "Change has been set aside and is deducted from available cash."
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        <div className="p-4 rounded-lg bg-secondary space-y-2">
                                            <div className="flex justify-between items-center">
                                                <p className="text-sm text-muted-foreground">Expected MoMo</p>
                                                <p className="font-semibold">{formatCurrency(expectedMomo)}</p>
                                            </div>
                                            <div className="flex justify-between items-center">
                                                <p className="text-sm text-muted-foreground">Counted MoMo</p>
                                                <p className="font-semibold">{formatCurrency(totalCountedMomo)}</p>
                                            </div>
                                            <Separator />
                                            <div className={`flex justify-between items-center font-bold ${floatsEqual(momoBalanceDifference, 0)
                                                ? 'text-green-600'
                                                : 'text-red-600'
                                                }`}>
                            ...
                            <Card className={`border-2 ${balanceStatus.bg}`}>
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-center space-x-3">
                                        <BalanceIcon className={`h-6 w-6 ${balanceStatus.color}`} />
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <Textarea
                                        ...
                                        placeholder="Add any notes for discrepancy..."
                                    />
                                </CardContent>
                            </Card>
