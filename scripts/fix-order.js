...
async function main() {
  ...
  try {
    ...
    let snapshot = await db.collection('orders').where('simplifiedId', '==', orderId).get();
    ...
    if (snapshot.empty) {
      ...
      recentOrders.docs.forEach(doc => {
        ...
        console.log(`  ${order.simplifiedId || doc.id} - ${order.tag || 'No tag'} - ${order.paymentStatus} - Total: ${order.total}`);
      });
